...
API.signTxp = function(txp, derivedXPrivKey) {
  ...
  var SIGHASH=Bitcore.crypto.Signature.SIGHASH_ALL;
  if(txp.network==='bcctestnet' || txp.network==='bcclivenet') {
    console.log("API.signTxp: adding FORKID");
    SIGHASH = Bitcore.crypto.Signature.SIGHASH_ALL | Bitcore.crypto.Signature.SIGHASH_FORKID;
  }

  var signatures = _.map(privs, function(priv, i) {
    return t.getSignatures(priv,SIGHASH);
  });
...
API.prototype.getFeeLevels = function(network, cb) {
  ...
  $.checkArgument(network || _.includes(['livenet', 'testnet','bcclivenet', 'bcctestnet'], network));
...
API.prototype.createWallet = function(walletName, copayerName, m, n, opts, cb) {
  ...
  if (!_.includes(['testnet', 'livenet','bcctestnet', 'bcclivenet'], network)) return cb(new Error('Invalid network'));
