...
function _checkNetwork(network) {
  if (!_.includes(['livenet', 'testnet','bcclivenet', 'bcctestnet'], network)) throw new Error('Invalid network');
};
...
Credentials.prototype._expand = function() {
  ...
  if (this.network) {
    $.checkState(this.network.endsWith(network));
...
Credentials.prototype.getBaseAddressDerivationPath = function() {
  ...
  var coin = ((this.network == 'livenet' || this.network == 'bcclivenet') ? "0" : "1");
